...
abstract class AdminCache extends ContainerAware {
   ...
   /**
    ...
    public function getAction( $action_name ){
        if( is_object($action_name) ) {
            if( $action_name instanceof ActionCache ) {
                $_action    = $this->getAction( $action_name->getName() ) ;
                if( $_action !== $action_name ) {
                    throw new \Exception( sprintf("`%s`:`%s` getAction exprect string parameter, but get %s:%s", $this->name, $this->class_name
                            , $action_name->getAdmin()->getName(), $action_name->getName() ) ) ;
                }
                return $action_name ;
            } else {
                throw new \Exception( sprintf("`%s`:`%s` getAction exprect string parameter", $this->name, $this->class_name ) ) ;
            }
        }
