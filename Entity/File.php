...
class File
{
    ...
    static public function getRichTextFilesPattern($web_assets_dir){
        $path = preg_quote($web_assets_dir, '/' ) ;
        return '/(?<=\W)' . $path . '\/upload\/html\/([0-9a-f]{8}\-[0-9a-f]{4}\-[0-9a-f]{4}\-[0-9a-f]{4}\-[0-9a-f]{12})\.(\w+)(?=\W)/' ;
    }

    static public function getFilesPattern($web_assets_dir){
        $path = preg_quote($web_assets_dir, '/' ) ;
        return '/^'. $path . '\/upload\/file\/([\w\-]+)\.\w+/' ;
    }

    static function getAllFilesPattern($web_assets_dir){
        $path = preg_quote($web_assets_dir, '/' ) ;
        return '/^'. $path . '\/upload\/(html|file)\/([0-9a-f]{8}\-[0-9a-f]{4}\-[0-9a-f]{4}\-[0-9a-f]{4}\-[0-9a-f]{12})\.(\w+)$/' ;
    }
